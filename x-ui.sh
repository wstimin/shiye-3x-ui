...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/wstimin/shiye-3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/wstimin/shiye-3x-ui/v${ver}/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/wstimin/shiye-3x-ui/v${ver}/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using main${plain}" >&2
        echo "https://raw.githubusercontent.com/wstimin/shiye-3x-ui/main/x-ui.sh"
    fi
}
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/wstimin/shiye-3x-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/wstimin/shiye-3x-ui/master/install.sh)${plain}"
    echo ""
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            ...
            1)
                ...
                ;;
            *)
                echo -e "${red}选项无效，请输入正确的编号。${plain}\n"
                show_log
                ;;
        esac
    else
        echo -e "${green}\t1.${plain} 调试日志"
        echo -e "${green}\t2.${plain} 清除全部日志"
        echo -e "${green}\t0.${plain} 返回主菜单"
        read -rp "请选择：" choice
...
update_geo() {
    echo -e "${green}\t1.${plain} Loyalsoldier 数据源（geoip.dat、geosite.dat）"
    echo -e "${green}\t2.${plain} chocolate4u 数据源（伊朗区域）"
    echo -e "${green}\t3.${plain} runetfreedom 数据源（俄罗斯区域）"
...
postgresql_menu() {
    echo -e "${green}\t1.${plain} ${green}安装${plain} PostgreSQL（服务端、客户端和 x-ui 数据库）"
    echo -e "${green}\t2.${plain} 迁移 SQLite ${green}->${plain} PostgreSQL"
    echo -e "${green}\t3.${plain} 查看状态（集群和 5432 端口）"
...
# Customer portal helper. Billing and provider values are edited in the
# authenticated panel API; this menu intentionally does not write credentials
# or secrets into the database from a root shell.
customer_portal_menu() {
    while true; do
        ...
        echo -e "  0. 返回上一级"
        read -rp "请输入选项 [0-3]: " portal_choice
        case "$portal_choice" in
            1)
                ...
                for ip_address in "https://api4.ipify.org" "https://ipv4.icanhazip.com" "https://4.ident.me"; do
                    response=$(curl -s --max-time 3 "$ip_address" 2>/dev/null | tr -d '[:space:]')
            ...
            2)
                ...
                echo -e "客户域名请使用本菜单的第 3 项生成 Nginx 配置，后台端口不会被修改。"
                ;;
            3)
                customer_portal_proxy_menu
                ;;
            0) return 0 ;;
            *) echo -e "${red}选项无效，请重新输入。${plain}" ;;
        esac
    done
}

# Generate a portal-only reverse proxy. The existing XUI listener, port and
# admin URL remain unchanged. Nginx exposes only /portal, its API and the
# assets needed by portal.html; panel routes are returned as 404 on this host.
customer_portal_proxy_menu() {
    ...
    if [[ "$base_path" == "" && -f "$cert_file" && -f "$key_file" ]]; then
        cat > "$temp_conf" <<EOF
# Generated by x-ui customer portal menu. Admin listener and port are unchanged.
server {
    listen 80;
...
│  ${green}6.${plain} 卸载                                     │
│────────────────────────────────────────────────│
│  ${green}7.${plain} 重置账号密码                             │
│  ${green}8.${plain} 重置 Web 路径                            │
│  ${green}9.${plain} 重置设置                                 │
│  ${green}10.${plain} 修改端口                                │
...
│────────────────────────────────────────────────│
│  ${green}20.${plain} SSL 证书                                │
│  ${green}21.${plain} Cloudflare 证书                         │
│  ${green}22.${plain} IP 限制管理                              │
│  ${green}23.${plain} 防火墙管理                               │
│  ${green}24.${plain} SSH 端口转发                            │
│  ${green}25.${plain} PostgreSQL                              │
│────────────────────────────────────────────────│
│  ${green}26.${plain} 启用 BBR                                 │
│  ${green}27.${plain} 更新 Geo 文件                            │
│  ${green}28.${plain} 网络测速                                │
│  ${green}29.${plain} 客户门户管理                             │
╚────────────────────────────────────────────────╝
"
    show_status
    ...
    case "${num}" in
        ...
        28)
            ...
            ;;
        29)
            check_install && customer_portal_menu
            ;;
        *)
            LOGE "请输入正确的编号 [0-29]"
            ;;
