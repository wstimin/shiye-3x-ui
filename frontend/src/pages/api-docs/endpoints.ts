...
export const sections: readonly Section[] = [
...

  {
    ...
    endpoints: [
      ...
      {
        method: 'POST',
        path: '/panel/api/portal/customers/:username/delete',
        summary:
          'Delete a portal login and its wallet ledger rows. The xray client itself is untouched.',
        params: [{ name: 'username', in: 'path', type: 'string', desc: 'Portal username.' }],
      },
      {
        method: 'POST',
        path: '/panel/api/portal/coupons/generate',
        summary:
          'Mint a batch of pure-amount recharge codes. Only SHA-256 hashes persist; the plaintext codes are returned once for display/export.',
        body: '{\n  "count": 10,\n  "amountCents": 500,\n  "prefix": "VIP-",\n  "batchNo": "2026-09",\n  "expiresAt": 0\n}',
        response: '{\n  "success": true,\n  "obj": { "codes": ["VIP-xxxxxxxxxxxx"] }\n}',
      },
      {
        method: 'GET',
        path: '/panel/api/portal/coupons',
        summary:
          'List coupon batches newest first. Responses carry amount, status, batch and redemption metadata — never code hashes or plaintext.',
        response:
          '{\n  "success": true,\n  "obj": [\n    { "id": 1, "amountCents": 500, "status": "unused", "batchNo": "2026-09" }\n  ]\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/portal/coupons/:id/disable',
        summary: 'Void an unused recharge code so it can never be redeemed.',
        params: [{ name: 'id', in: 'path', type: 'number', desc: 'Coupon row ID.' }],
      },
      {
        method: 'GET',
