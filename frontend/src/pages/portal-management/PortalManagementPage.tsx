          ...
          <Layout.Content className="content-area portal-management-content">
                ...
                >
                  <Form form={billingForm} layout="vertical" onFinish={(v) => void saveBilling(v)}>
                    <Row gutter={16}>
                      <Col xs={24} sm={8}>
                        <Form.Item
                          ...
                          rules={[{ required: true, message: '请输入每月价格' }]}
                          extra="页面按元显示，系统内部换算为分保存"
                        >
                          <InputNumber
                            min={0}
                            precision={2}
                    ...
                    <Form.Item
                      label="续期月数选项（可选）"
                      name="plans"
                      extra="例如 [1,3,6]；价格按通用月价 × 月数计算。创建客户账号时绑定后台已有客户端邮箱，门户会显示该客户端对应的节点和订阅链接。"
                    >
                    ...
                    <Form.Item
                      label="卡密购买跳转链接"
                      name="purchaseUrl"
                      extra="这是用户点击购买卡密时跳转的链接，与三方接口地址独立"
                    >
                      <Input
                        prefix={<LinkOutlined />}
              ...
              <Col xs={24} xl={8}>
                <Card className="portal-management-card portal-management-summary">
                  ...
                  <Button
                    ...
                    type="primary"
                    icon={<LinkOutlined />}
                    href={`${window.X_UI_BASE_PATH || ''}portal`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    打开用户端
            ...
            >
              <Alert
                ...
                showIcon
                message="绑定关系"
                description="每个门户账号绑定一个后台客户端邮箱；该客户端所属的节点和订阅链接会显示给对应用户。月费是所有绑定客户端共用的通用价格。"
                style={{ marginBottom: 16 }}
              />
              <Table
                rowKey="username"
      ...
      >
        <Form form={customerForm} layout="vertical" onFinish={(v) => void createCustomer(v)}>
          ...
          <Form.Item
            ...
            name="email"
            rules={[{ required: true, message: '请输入已存在的客户端邮箱' }]}
          >
            <Input placeholder="必须是后台已有客户端邮箱" />
          </Form.Item>
        </Form>
      </Modal>
    </ConfigProvider>
