...
bash <(curl -Ls https://raw.githubusercontent.com/wstimin/shiye-3x-ui/master/install.sh) v3.4.0
```
...
docker run -d \
  ...
  --name 3x-ui \
  ghcr.io/wstimin/shiye-3x-ui:latest
```
...
[صفحهٔ releases](https://github.com/wstimin/shiye-3x-ui/releases) دانلود کنید، آن را
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/wstimin/shiye-3x-ui/master/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/wstimin/shiye-3x-ui/tree/main/deploy) در مخزن
