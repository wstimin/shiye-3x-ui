...
客户门户可以使用单独的域名，而不改变现有后台的端口和访问方式。运行 `x-ui` 菜单中的
`29. 客户门户管理` → `3. 设置/修改客户域名并生效`，脚本会读取当前 Web 基础路径和端口，
并生成 `/etc/nginx/conf.d/3x-ui-customer-portal.conf`。

客户域名只代理门户页面、`/portal/api/*` 和门户所需的静态资源；后台的 `/panel` 与后台 API
在该域名上返回 404。后台仍通过原有地址访问。详细部署步骤见仓库中的
`deploy/nginx/README-客户门户.md`。
